...
def face_swap(src_face, dst_face, src_points, dst_points, dst_shape, dst_img, args, end=48):
    ...
    ## 3d warp
    warped_src_face = warp_image_3d(src_face, src_points[48:65], dst_points[48:65], (h, w))
    # print('warped_src_face')
    # cv2.imwrite('warped_src_face.png', warped_src_face)
    ## Mask for blending
    mask = mask_from_points((h, w), dst_points)

    width = int(warped_src_face.shape[1] + 40)
    height = int(warped_src_face.shape[0] + 40)
    dim = (width, height)
    # resize image
    warped_src_face_xxx = cv2.resize(warped_src_face, dim, interpolation=cv2.INTER_AREA)
    mask_src = np.mean(warped_src_face_xxx, axis=2) > 0
    mask_src = mask_src[20:height-20, 20:width-20]
    ...
    ## 2d warp
    if args.warp_2d:
        unwarped_src_face = warp_image_3d(warped_src_face, dst_points[48:65], src_points[48:65], src_face.shape[:2])
